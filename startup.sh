...
docker run --name samdata_datasync -v /root/samterminal/datasync:/root/samterminal/datasync --network samdata_bridge -e EGG_SERVER_ENV=Client -e MySQL_Session=$MySQL_Information -e PostgreSQL_Session=$PostgreSQL_Information -p 17001:7001 -d samdataimage/datasync:$DataSync_Version
